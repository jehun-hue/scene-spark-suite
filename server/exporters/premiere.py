...
def generate_premiere_xml(analysis_json: dict, video_filename: str = "source.mp4") -> str:
    """Generates Premiere XML (FCP XML 7 format)."""
    ...
    for idx, item in enumerate(edit_list):
        ...
        ET.SubElement(clipitem, "name").text = item.get("subtitle_text", "Clip")
        ET.SubElement(clipitem, "duration").text = "90" # 3s @ 30fps
        ...
        # Simple subtitle generator if needed, but usually titles are separate tracks in Premiere XML
        # Adding some metadata to subtitle_text for reference
